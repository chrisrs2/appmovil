...
class Background(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        # crear texturas
        self.cloud_texture = Image(source="nubes.png").texture
        self.cloud_texture.wrap = 'repeat'
        self.cloud_texture.uvsize = (Window.width / self.cloud_texture.width, -1)

        self.floor_texture = Image(source="suelo.png").texture
    ...
    def scroll_textures(self, time_passed):
        # actualizar la uvpos de la textura
...
class MainApp(App):
    ...
    def check_collision(self):
        bird = self.root.ids.bird
        # se revisa las coliciones con tuberias
        is_colliding = False
        for pipe in self.pipes:
            if pipe.collide_widget(bird):
                is_colliding = True
                #se comprueba si no se sale del mapa
    ...
    # se termina el juego si hay colicion con tuberias o se sale del mapa
    def game_over(self):
    ...
    def move_pipes(self, time_passed, number):
        # mover tuberias
        for pipe in self.pipes:
            pipe.x -= time_passed * 100

        #se comprueba que siempre sigan las tuberias y no que se terminen 
        num_pipes = 4
        distance_between_pipes = Window.width / (num_pipes - 1)
